...
describe('emit', () => {
  ...
  it('writes a header without credentials and imports mocakit as a namespace', () => {
    expect(code).toMatch(/^\/\/ AUTO-GENERATED by mocakit 0\.1\.0 from https:\/\/moca\.test\/service — 10 commands\./);
  ...
  it('emits a repeated command once, keeping the lowest-sorting name, whatever the input order', () => {
    ...
    expect(forward.code).toMatch(/— 10 commands\./);
  ...
  it('returns the number of commands it emitted', () => {
    expect(emit(snapshot, { version: '0.1.0' }).count).toBe(10);
    const withDuplicate = { ...snapshot, commands: [...snapshot.commands, { name: 'EXEC', args: [] }] };
    expect(emit(withDuplicate, { version: '0.1.0' }).count).toBe(10);
  });
  ...
  describe('live-server argument rules', () => {
    ...
    describe('requiredness by command type', () => {
      const NOTE = 'marked required by MOCA; not enforced for Local Syntax commands';
      const typed = (type: string | undefined, args: Snapshot['commands'][number]['args']) =>
        emit({ ...snapshot, commands: [{ name: 'cmd', ...(type === undefined ? {} : { type }), args }] }, { version: '0.1.0' });

      it('makes a flagged argument optional on a Local Syntax command, with a JSDoc note', () => {
        const result = typed('Local Syntax', [
          { name: 'usr_id', dtype: 'STRING', required: true, description: 'User' },
          { name: 'wh_id', dtype: 'STRING', required: false },
        ]);
        expect(result.code).toContain(`  /** User (STRING) · ${NOTE} */\n  usr_id?: string | null;`);
        expect(result.code).toContain('  /** (STRING) */\n  wh_id?: string | null;');
        expect(result.code).toContain('  cmd: ["cmd", [["usr_id", "STRING", 0], ["wh_id", "STRING", 0]]],');
        expect(result.code).toContain('  cmd: mk.OptionalArgsCommand<CmdArgs, "cmd">;');
        expect(result.warnings).toEqual([]);
      });

      it('adds the note to a flagged Local Syntax argument with no description', () => {
        const result = typed('Local Syntax', [{ name: 'a', dtype: 'STRING', required: true }]);
        expect(result.code).toContain(`  /** (STRING) · ${NOTE} */\n  a?: string | null;`);
      });

      it('matches Local Syntax ignoring case and surrounding whitespace', () => {
        for (const type of ['local syntax', '  LOCAL SYNTAX ', 'Local syntax']) {
          const result = typed(type, [{ name: 'a', dtype: 'STRING', required: true }]);
          expect(result.code, type).toContain('  cmd: ["cmd", [["a", "STRING", 0]]],');
          expect(result.code, type).toContain('  cmd: mk.OptionalArgsCommand<CmdArgs, "cmd">;');
        }
      });

      it('keeps flagged arguments required on compiled commands and when the type is missing', () => {
        for (const type of ['C Function', 'Simple C Function', 'Java Method', 'java method', undefined]) {
          const result = typed(type, [
            { name: 'a', dtype: 'STRING', required: true },
            { name: 'b', dtype: 'STRING', required: false },
          ]);
          expect(result.code, String(type)).toContain('  a: string;');
          expect(result.code, String(type)).toContain('  cmd: ["cmd", [["a", "STRING", 1], ["b", "STRING", 0]]],');
          expect(result.code, String(type)).toContain('  cmd: mk.Command<CmdArgs, "cmd">;');
          expect(result.code, String(type)).not.toContain(NOTE);
        }
      });
      ...
      it('drops a flagged stack argument on a Local Syntax command instead of skipping it', () => {
        const result = typed('Local Syntax', [
          { name: '@res', dtype: 'RESULTS', required: true },
          { name: 'a', dtype: 'STRING', required: false },
        ]);
        expect(result.count).toBe(1);
        expect(result.warnings).toEqual([]);
        expect(result.code).toContain('  cmd: ["cmd", [["a", "STRING", 0]]],');
        expect(result.code).toContain('   * Stack-only arguments not settable here: res (RESULTS)');
        expect(result.code).toContain('  cmd: mk.OptionalArgsCommand<CmdArgs, "cmd">;');
      });

      it('still skips a compiled or untyped command with a flagged stack argument', () => {
        for (const type of ['Java Method', undefined]) {
          const result = typed(type, [{ name: '@res', dtype: 'RESULTS', required: true }]);
          expect(result.count, String(type)).toBe(0);
          expect(result.warnings, String(type)).toEqual([
            'Command "cmd" requires stack argument "res" (RESULTS); skipped (run it with exec())',
          ]);
        }
      });

      it('drops, rather than skips for, a flagged invalid argument name on a Local Syntax command', () => {
        const result = typed('Local Syntax', [{ name: 'wh-id', dtype: 'STRING', required: true }]);
        expect(result.count).toBe(1);
        expect(result.warnings).toEqual([
          'Command "cmd" argument "wh-id" is not a valid MOCA argument name; dropped the argument',
        ]);
      });

      it('applies to the fixture commands', () => {
        expect(code).toContain(`  /** Gadget (STRING) · ${NOTE} */\n  gadget_id?: string | null;`);
        expect(code).toContain('  listGadgetBins: ["list gadget bins", [["gadget_id", "STRING", 0]]],');
        expect(code).toContain('  listGadgetBins: mk.OptionalArgsCommand<ListGadgetBinsArgs, "list gadget bins">;');
        expect(code).toContain('  validateGadget: ["validate gadget", [["gadget_id", "STRING", 1]]],');
        expect(code).toContain('  validateGadget: mk.Command<ValidateGadgetArgs, "validate gadget">;');
      });
    });
  });
});
